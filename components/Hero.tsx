  ...
  return (
    <section className="relative flex min-h-[100svh] items-center justify-center overflow-hidden text-center">
      {/* Full-bleed cinematic background video (z-0 so the section's content,
          ...
          Loads on bone, then the video fades in. */}
      <div className="absolute inset-0 z-0 overflow-hidden bg-bone">
        ...
        />
        {/* Bone scrims keep the dark headline legible while showing the footage */}
        <div className="absolute inset-0 bg-bone/45" />
        <div className="absolute inset-0 bg-gradient-to-b from-bone/70 via-bone/30 to-bone/80" />
      </div>

      <div className="container-c relative z-10 px-6 py-32 md:px-10">
        <p className="mb-7 animate-fade-up text-xs font-medium uppercase tracking-widest2 text-slate">
          Trusted by industry leaders across Australia
        </p>
        <h1 className="mx-auto max-w-5xl animate-fade-up display-xl text-ink [text-shadow:0_1px_18px_rgba(245,241,232,0.6)]">
          Content That Converts
        </h1>
        <p
          className="mx-auto mt-7 max-w-4xl animate-fade-up text-balance text-lg leading-relaxed text-ink/80 [text-shadow:0_1px_10px_rgba(245,241,232,0.5)] md:text-xl"
      ...
      <div className="absolute bottom-7 left-1/2 z-10 -translate-x-1/2">
        <span className="block h-2 w-9 rounded-full bg-ink/30" />
      </div>
    </section>
  );
}
